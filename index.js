...
class Strategy {
    ...
    init() {
        return this.createNewContract().then((result)=>console.log(result)).catch((error) => {
        ...
        const subscriber = contract.onUpdate(() => {
            if (contract.status === 'proposal' && !this.purchased) {
                if (this.buyCondition(contract)) {
                    this.purchased = true;
                    this.buy(contract).then((result)=>console.log(result)).catch(console.log);
                }
            } else {
                if (contract.is_open && this.sellCondition(contract)) {
                    this.sell(contract).then((result)=>console.log(result)).catch(console.log);
...
async function main () {
    const symbols = await Promise.all([
        {symbol: 'stpRNG', pip: 3, mult: 500, type: 'MULTUP'  , ratio: 0.10 },
        {symbol: 'stpRNG', pip: 3, mult: 250, type: 'MULTUP'  , ratio: 0.25 },
        {symbol: 'stpRNG', pip: 4, mult: 100, type: 'MULTUP'  , ratio: 0.50 },
        {symbol: 'stpRNG', pip: 4, mult: 75 , type: 'MULTUP'  , ratio: 0.75 },
        {symbol: 'stpRNG', pip: 2, mult: 500, type: 'MULTUP'  , ratio: 1.00},
        {symbol: 'stpRNG', pip: 3, mult: 500, type: 'MULTDOWN', ratio: 0.10 },
        {symbol: 'stpRNG', pip: 3, mult: 250, type: 'MULTDOWN', ratio: 0.25 },
        {symbol: 'stpRNG', pip: 4, mult: 100, type: 'MULTDOWN', ratio: 0.50 },
        {symbol: 'stpRNG', pip: 4, mult: 75 , type: 'MULTDOWN', ratio: 0.75 },
        {symbol: 'stpRNG', pip: 2, mult: 50, type: 'MULTDOWN', ratio: 1.00 },
